    ...
    <Card className="p-8 glass-card card-elevated">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {stories.map((story, index) => (
          <Card
            key={index}
            className="p-6 glass-effect card-elevated glow-on-hover border-[#78B288]/30 hover:border-[#78B288]/50 transition-all duration-300"
          >
            <div className="text-4xl mb-4">{story.icon}</div>
            <h3 className="text-lg font-semibold mb-2">{story.title}</h3>
            <p className="text-sm text-muted-foreground mb-4">{story.description}</p>
            <Badge variant="outline" className="text-[#78B288] border-[#78B288] shimmer">
              {story.impact}
            </Badge>
          </Card>
        ))}
      ...
      <div className="mt-8 p-6 glass-effect rounded-lg border border-[#78B288]/20">
